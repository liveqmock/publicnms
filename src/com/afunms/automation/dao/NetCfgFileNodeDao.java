...
public class NetCfgFileNodeDao extends BaseDao implements DaoInterface {
	...
	public boolean isExistsIp(String ipAddresses) {
		int tmp = -1;
		String sql = "select count(*) from automation_node where ip_address in ('"
				+ ipAddresses.replace(",", "','") + "')";
	...
	/**
	 * 查询：有条件分页查询远程登录设备的方法
	 * new add wangwenbo 2014年9月15日 10:13:30
	 */
	@SuppressWarnings("unchecked")
	public List<NetCfgFileNode> listByPage(int curpage,String where,int perpage){
		List<NetCfgFileNode> ncfnList = new ArrayList<NetCfgFileNode>();
		try{
			ncfnList =(List<NetCfgFileNode>)super.listByPage(curpage, where, perpage);
		}catch(Exception e){
			e.printStackTrace();
			System.out.println("查询：有条件分页查询远程登录设备的方法...失败");
		}
		return ncfnList;
	}
	...
	public int selectCount(){
		String sql = "select count(*) from automation_node";
		int count = 0;
		try{
	...
	public void deleteByIp(String ipAddress) {
		conn.executeUpdate("delete from automation_node where ip_address='"
				+ ipAddress + "'");
	}
	...
	public List loadEnableVpn() {
		...
		try {
			rs = conn
					.executeQuery("select * from automation_node where enablevpn=1 order by id");
			while (rs.next())
				list.add(loadFromRS(rs));
		} catch (Exception e) {
	...
	public List loadByIds(String[] ids) {
		...
		try {
			StringBuffer sql = new StringBuffer();
			sql.append("select * from automation_node where id in("
					+ split.substring(1) + ")");
			rs = conn.executeQuery(sql.toString());
			while (rs.next()) {
				list.add(loadFromRS(rs));
	...
	public boolean save(BaseVo basevo) {
		...
		sql.append(vo.getAlias());
		sql.append("')");
		System.out.println("增加远程登录设备正在保存中。。。。");
		return saveOrUpdate(sql.toString());
	}
	...
	public List find(String ipaddress) {
		return findByCriteria("select * from automation_node k where k.ip_address like '%"
				+ ipaddress + "%';");
	}
	...
	public String findbyip(String ipaddress) {
		...
		try {
			rs = conn
					.executeQuery("select * from automation_node where ip_address='"
							+ ipaddress + "';");
			while (rs.next()) {
